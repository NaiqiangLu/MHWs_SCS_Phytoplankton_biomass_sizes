...
mFpico=squeeze(climFpico(:,:,loc_plot));  %2003–2024 每一天对应的 climatology Chla,同时用于去掉长度为 1 的维度。
...
RFpico = nan(size(Fpico,1),size(Fpico,2));
...
for m = 1:size(loc_full,1)
    ...
    rfpico = nan(size(period_mhw,1),1);  %一定要初始化矩阵，不然数据会被覆盖
    for loc = 1:size(period_mhw,1)
        ...
        % ===== 提取数据 =====
        mhw_pico = squeeze(Fpico(loc_here(1),loc_here(2), ...
            mhw_time - datenum(1998,1,1) + 1));

        clim_pico = squeeze(mFpico(loc_here(1),loc_here(2), ...
            mhw_time - datenum(1998,1,1) + 1));

        % ===== 单次事件 anomaly =====
      
        rfpico(loc) = nanmean((mhw_pico - clim_pico).*100);
    end
...
% ===== 绘制异常 =====
m_contourf(Lon, Lat, RFpico', 80, 'linestyle', 'none'); % 注意转置
shading interp;
...
set(cb,'linewidth',1.5,'fontsize',16,'edgecolor','k','YTick',[-6:2:6]);
set(get(cb,'ylabel'),'string','Fpico anomaly (%)','FontSize',24,'FontName','Times New Roman');
% title(cb,'(%)','FontSize',16,'FontName','Times New Roman');
% title('Fpico Anomaly during MHWs','FontSize',18,'FontWeight','bold');
