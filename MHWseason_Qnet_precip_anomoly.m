...
load Clim_precipitation.mat
Rain_sum = permute(Rain,[2,1,3]); 
...
mQnet=squeeze(Qnet_clim(:,:,loc_plot));  %1998–2024 每一天对应的 climatology Chla,同时用于去掉长度为 1 的维度。
mPrecip=squeeze(Precip_clim(:,:,loc_plot)); 
...
for seasonal=1:size(season,1)        %seasons是一个4*3的矩阵，返回行数 第一行春季（3 4 5月）  按循环四季
    ...
    VPrecip = nan(size(Rain_sum,1),size(Rain_sum,2));
    SigMask  = nan(size(Qnet,1),size(Qnet,2));
    for m=1:size(loc_full,1)             %逐个处理每个网格点
        ...
        vqnet =nan(size(period_mhw,1),1);  %初始化空矩阵
        ...
        for loc = 1:size(period_mhw,1)
            ...
            vqnet(loc) = nanmean(mhw_qnet-clim_qnet);  %计算异常值


            mhw_precip = squeeze(Rain_sum(loc_here(1),loc_here(2),...
                mhw_time-datenum(1998,1,1)+1));  %提取mhw期间chla   提取单次mhw事件的chla
    ...
    MHWs_Qnet_Season{seasonal} = VQnet;      %异常值
    MHWs_Precipval_Season{seasonal} = VPrecip;     %四个季节降雨的异常值
...
m_contourf(Lon,Lat,MHWs_Qnet_Season{1}',50,'linestyle','none');
shading flat      % 建议用flat，方便保持色阶分明

hold on
% ===== 降雨量异常等值线 =====
Rain_sum = MHWs_Precipval_Season{1}';
neg_levels = -6:2:-1;
pos_levels = 1:2:6;
% 负降雨异常（偏干）
m_contour(Lon,Lat,Rain_sum,...
    neg_levels,...
...
m_scatter(Lon(i),Lat(j),1.5,'k','filled');
%海岸线
...
set(cb,'linewidth',1.5,'fontsize',18,'edgecolor','k','ytick',[-60:30:60]);
set(get(cb,'ylabel'),'string','Qnet (W m^{-2})','FontSize',18,'FontName','Times New Roman');%生成colorbar标题，单位
% ===== colorbar分割黑线 =====
drawnow
pos = cb.Position;
for k = 2:14
...
figure('Position',[200,200,900,800],'Resize','off','Color','w');
m_proj('equidistant','lon',[105 123],'lat',[0 25]);
m_contourf(Lon,Lat,MHWs_Qnet_Season{2}',50,'linestyle','none');
shading flat      % 建议用flat，方便保持色阶分明

hold on
% ===== 降雨量异常等值线 =====
Rain_sum = MHWs_Precipval_Season{2}';
neg_levels = -6:2:-1;
pos_levels = 1:2:6;
% 负降雨异常（偏干）
m_contour(Lon,Lat,Rain_sum,...
...
figure('Position',[200,200,900,800],'Resize','off','Color','w');
m_proj('equidistant','lon',[105 123],'lat',[0 25]);
m_contourf(Lon,Lat,MHWs_Qnet_Season{3}',50,'linestyle','none');
shading flat      % 建议用flat，方便保持色阶分明
...
figure('Position',[200,200,900,800],'Resize','off','Color','w');
m_proj('equidistant','lon',[105 123],'lat',[0 25]);
m_contourf(Lon,Lat,MHWs_Qnet_Season{4}',50,'linestyle','none');
shading flat      % 建议用flat，方便保持色阶分明
...
%春季
B_region1 = MHWs_Qnet_Season{1}(ix2, iy2);
B_region1 = B_region1(:);
B_region1 = B_region1(~isnan(B_region1));
%夏季
B_region2 = MHWs_Qnet_Season{2}(ix2, iy2);
B_region2 = B_region2(:);
B_region2 = B_region2(~isnan(B_region2));
%秋季
B_region3 = MHWs_Qnet_Season{3}(ix2, iy2);
B_region3 = B_region3(:);
B_region3 = B_region3(~isnan(B_region3));
%冬季
B_region4 = MHWs_Qnet_Season{4}(ix2, iy2);
B_region4 = B_region4(:);
