...
mFmicro = squeeze(climFmicro(:,:,loc_plot));  %2003–2024 每一天对应的 climatology Chla,同时用于去掉长度为 1 的维度。
...
RFmicro = nan(size(Fmicro,1),size(Fmicro,2));
...
for m = 1:size(loc_full,1)
    ...
    rfmicro = nan(size(period_mhw,1),1);  %一定要初始化矩阵，不然数据会被覆盖
    for loc = 1:size(period_mhw,1)
        ...
        % ===== 提取数据 =====
        mhw_micro = squeeze(Fmicro(loc_here(1),loc_here(2), ...
            mhw_time - datenum(1998,1,1) + 1));

        clim_micro = squeeze(mFmicro(loc_here(1),loc_here(2), ...
            mhw_time - datenum(1998,1,1) + 1));

        % ===== 单次事件 anomaly =====
        vfmicro(loc) = nanmean(mhw_micro - clim_micro);
        rfmicro(loc) = nanmean((mhw_micro - clim_micro).*100);
    end
...
% ===== 绘制异常百分比 =====
m_contourf(Lon, Lat, RFmicro', 80, 'linestyle', 'none');  %注意转置
shading interp
...
% ===== 配色方案 =====
colormap(nclCM(227,18));  %zhan et al 同款配色
caxis([-6   6]);        % 设置颜色范围
...
% set(get(cb,'ylabel'),'string','Fmicro anomaly (%)','FontSize',20,'FontName','Times New Roman');
% title(cb,'(%)','FontSize',14,'FontName','Times New Roman');
% title('Fmicro Anomaly during MHWs','FontSize',18,'FontWeight','bold');
