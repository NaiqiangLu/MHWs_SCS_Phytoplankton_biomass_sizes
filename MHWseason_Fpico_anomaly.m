...
Fpico(Fpico<0)=nan;   %变量去异常值
Fpico = permute(Fpico,[2 1 3]); %转置数据 交换维度，把叶绿素矩阵正常投影

load ClimFphyto1998-2024.mat     %加载三组分叶绿素每日气候态数据；climchla为变量名
climFpico=permute(climFpico,[2,1,3]);
...
mFpico=squeeze(climFpico(:,:,loc_plot));  %1998–2024 每一天对应的 climatology Chla,同时用于去掉长度为 1 的维度。
...
for seasonal=1:size(season,1)        %seasons是一个4*3的矩阵，返回行数 第一行春季（3 4 5月）  按循环四季
    ...
    RFPico = nan(size(Fpico,1),size(Fpico,2));  %初始化异常值矩阵
    SigMask  = nan(size(Fpico,1),size(Fpico,2));
    for m=1:size(loc_full,1)             %逐个处理每个网格点
        ...
        rfpico  = nan(size(period_mhw,1),1);
        for loc = 1:size(period_mhw,1)

            mhw_time = period_mhw(loc,1):period_mhw(loc,2);
            tidx = mhw_time - studytime(1) + 1;

            mhw_fpico  = squeeze(Fpico(loc_here(1),loc_here(2),tidx));
            clim_fpico = squeeze(mFpico(loc_here(1),loc_here(2),tidx));

            rfpico(loc)  = nanmean((mhw_fpico-clim_fpico).*100);

        end

        % 至少1个mhw事件的值才能做平均
        if sum(~isnan(rfpico)) >= 1
            RFPico(loc_here(1),loc_here(2)) = nanmean(rfpico);         
        end

        % t-test
        if sum(~isnan(rfpico)) > 1    %至少有两个有效值来做t检验
            [h,p] = ttest(rfpico,0,'Alpha',0.05);
            if h==1
    ...
    MHWs_FPico_Season{seasonal} = RFPico;  %PFTs异常的百分比
end
toc
...
m_contourf(Lon,Lat,MHWs_FPico_Season{1}',80,'linestyle','none');
shading interp
hold on;
% ===== 显著性黑点 =====
...
set(get(cb,'ylabel'),'string','Fpico anomaly (%)','FontSize',24,'FontName','Times New Roman');%生成colorbar标题，单位
% title(cb,'(%)','FontSize',18,'FontName','Times New Roman');
% title('Fpico (Xi et al.2021)','FontSize',18,'FontName','Times New Roman');


%%% PFTs summer
figure('Position',[200,200,900,800],'Color','w');
m_proj('equidistant','lon',[105 123],'lat',[0 25]);
m_contourf(Lon,Lat,MHWs_FPico_Season{2}',80,'linestyle','none');
shading interp
hold on
% ===== 显著性黑点 =====
...
figure('Position',[200,200,900,800],'Color','w');
m_proj('equidistant','lon',[105 123],'lat',[0 25]);
m_contourf(Lon,Lat,MHWs_FPico_Season{3}',80,'linestyle','none');
shading interp
hold on
% ===== 显著性黑点 =====
...
figure('Position',[200,200,900,800],'Color','w');
m_proj('equidistant','lon',[105 123],'lat',[0 25]);
m_contourf(Lon,Lat,MHWs_FPico_Season{4}',80,'linestyle','none');
shading interp
hold on
% ===== 显著性黑点 =====
...
%春季
B_region1 = MHWs_FPico_Season{1}(ix2, iy2);
B_region1 = B_region1(:);
B_region1 = B_region1(~isnan(B_region1));
%夏季
B_region2 = MHWs_FPico_Season{2}(ix2, iy2);
B_region2 = B_region2(:);
B_region2 = B_region2(~isnan(B_region2));
%秋季
B_region3 = MHWs_FPico_Season{3}(ix2, iy2);
B_region3 = B_region3(:);
B_region3 = B_region3(~isnan(B_region3));
%冬季
B_region4 = MHWs_FPico_Season{4}(ix2, iy2);
B_region4 = B_region4(:);
