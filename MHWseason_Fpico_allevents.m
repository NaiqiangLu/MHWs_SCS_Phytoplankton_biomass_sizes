%% 提取 Region1(夏季) 和 Region2(冬季) 每次MHW事件的Fpico比例异常
clear;clc;close all;
...
mFpico = squeeze(climFpico(:,:,loc_plot));
...
for seasonal = 1:4
    ...
    for m = 1:size(loc_full,1)
        ...
        for loc = 1:size(period_mhw,1)
            ...
            mhw_fpico = squeeze(...
                Fpico(loc_here(1),loc_here(2),tidx));

            clim_fpico = squeeze(...
                mFpico(loc_here(1),loc_here(2),tidx));
