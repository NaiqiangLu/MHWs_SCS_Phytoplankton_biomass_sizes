%% 提取 Region1(夏季) 和 Region2(冬季) 每次MHW事件的MLD异常值
clear;clc;close all;
...
for seasonal = 1:4
    ...
    for m = 1:size(loc_full,1)
        ...
        for loc = 1:size(period_mhw,1)
            ...
            mhw_mld = squeeze(...
                MLD(loc_here(1),loc_here(2),...
                mhw_time-datenum(1998,1,1)+1));

            clim_mld = squeeze(...
                mMLD(loc_here(1),loc_here(2),...
                mhw_time-datenum(1998,1,1)+1));
            ...
            %% 当前MHW事件MLD异常值
            vmld(loc) = nanmean(mhw_mld-clim_mld);
