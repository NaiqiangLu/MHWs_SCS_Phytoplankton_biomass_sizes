...
SSR = permute(SSR,[2 1 3]); %转置数据 交换维度，把叶绿素矩阵正常投影

load Clim_Qnet_SSR_STR_SLHF_SSHF.mat   %加载叶绿素每日气候态数据；climchla为变量名
SSR_clim = permute(SSR_clim,[2,1,3]);
...
mSSR=squeeze(SSR_clim(:,:,loc_plot));  %1998–2024 每一天对应的 climatology Chla,同时用于去掉长度为 1 的维度。
...
for seasonal=1:size(season,1)        %seasons是一个4*3的矩阵，返回行数 第一行春季（3 4 5月）  按循环四季
    ...
    VSSR = nan(size(SSR,1),size(SSR,2));
    SigMask  = nan(size(SSR,1),size(SSR,2));
    for m=1:size(loc_full,1)             %逐个处理每个网格点
        ...
        vssr =nan(size(period_mhw,1),1);  %初始化空矩阵    
        for loc = 1:size(period_mhw,1)
            % 时间段筛选
            mhw_time = period_mhw(loc,1):period_mhw(loc,2);

            mhw_ssr = squeeze(SSR(loc_here(1),loc_here(2),...
                mhw_time-datenum(1998,1,1)+1));  %提取mhw期间chla   提取单次mhw事件的chla

            clim_ssr = squeeze(mSSR(loc_here(1),loc_here(2),...
                mhw_time-datenum(1998,1,1)+1));  %提取气候态chla     提取单次mhw事件期间的气候态chla
            
            vssr(loc) = nanmean(mhw_ssr-clim_ssr);  %计算异常值
        end

        % 对异常值进行平均 
        VSSR(loc_here(1),loc_here(2)) = nanmean(vssr);

        % t-test
        if sum(~isnan(vssr)) > 1
            [h,p] = ttest(vssr,0,'Alpha',0.05);
            if h==1
    ...
    MHWs_SSR_Season{seasonal} = VSSR;      %异常值
...
m_contourf(Lon,Lat,MHWs_SSR_Season{1}',80,'linestyle','none');
shading flat      % 建议用flat，方便保持色阶分明
hold on
% ===== 添加等值线（无数字）=====
contour_levels = -30:20:30;   % 等值线间隔可调整
m_contour(Lon,Lat,MHWs_SSR_Season{1}',contour_levels,...
    'LineColor','k',...
    'LineWidth',0.8);
...
set(cb,'linewidth',1.5,'fontsize',18,'edgecolor','k','ytick',[-30:15:30]);
set(get(cb,'ylabel'),'string','SSR (W m^{-2})','FontSize',18,'FontName','Times New Roman');%生成colorbar标题，单位
% ===== colorbar分割黑线 =====
drawnow
pos = cb.Position;
for k = 2:14
...
figure('Position',[200,200,900,800],'Resize','off','Color','w');
m_proj('equidistant','lon',[105 123],'lat',[0 25]);
m_contourf(Lon,Lat,MHWs_SSR_Season{2}',80,'linestyle','none');
shading flat      % 建议用flat，方便保持色阶分明
hold on
% ===== 添加等值线（无数字）=====
contour_levels = -30:20:30;   % 等值线间隔可调整
m_contour(Lon,Lat,MHWs_SSR_Season{2}',contour_levels,...
    'LineColor','k',...
    'LineWidth',0.8);
...
figure('Position',[200,200,900,800],'Resize','off','Color','w');
m_proj('equidistant','lon',[105 123],'lat',[0 25]);
m_contourf(Lon,Lat,MHWs_SSR_Season{3}',80,'linestyle','none');
shading flat      % 建议用flat，方便保持色阶分明
hold on
% ===== 添加等值线（无数字）=====
contour_levels = -30:20:30;   % 等值线间隔可调整
m_contour(Lon,Lat,MHWs_SSR_Season{3}',contour_levels,...
    'LineColor','k',...
    'LineWidth',0.8);
...
figure('Position',[200,200,900,800],'Resize','off','Color','w');
m_proj('equidistant','lon',[105 123],'lat',[0 25]);
m_contourf(Lon,Lat,MHWs_SSR_Season{4}',80,'linestyle','none');
shading flat      % 建议用flat，方便保持色阶分明
hold on
% ===== 添加等值线（无数字）=====
contour_levels = -30:20:30;   % 等值线间隔可调整
m_contour(Lon,Lat,MHWs_SSR_Season{4}',contour_levels,...
    'LineColor','k',...
    'LineWidth',0.8);
...
%春季
B_region1 = MHWs_SSR_Season{1}(ix2, iy2);
B_region1 = B_region1(:);
B_region1 = B_region1(~isnan(B_region1));
%夏季
B_region2 = MHWs_SSR_Season{2}(ix2, iy2);
B_region2 = B_region2(:);
B_region2 = B_region2(~isnan(B_region2));
%秋季
B_region3 = MHWs_SSR_Season{3}(ix2, iy2);
B_region3 = B_region3(:);
B_region3 = B_region3(~isnan(B_region3));
%冬季
B_region4 = MHWs_SSR_Season{4}(ix2, iy2);
B_region4 = B_region4(:);
