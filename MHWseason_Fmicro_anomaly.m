...
Fmicro = permute(Fmicro,[2 1 3]); %转置数据 交换维度，把叶绿素矩阵正常投影
...
mFmicro = squeeze(climFmicro(:,:,loc_plot));  %1998–2024 每一天对应的 climatology Chla,同时用于去掉长度为 1 的维度。
...
for seasonal=1:size(season,1)        %seasons是一个4*3的矩阵，返回行数 第一行春季（3 4 5月）  按循环四季
    ...
    RFMicro = nan(size(Fmicro,1),size(Fmicro,2));  %初始化异常值矩阵   
    SigMask  = nan(size(Fmicro,1),size(Fmicro,2));
    for m=1:size(loc_full,1)             %逐个处理每个网格点
        ...
        vfmicro = nan(size(period_mhw,1),1);
        ...
        for loc = 1:size(period_mhw,1)
            ...
            mhw_fmicro  = squeeze(Fmicro(loc_here(1),loc_here(2),tidx));
            clim_fmicro = squeeze(mFmicro(loc_here(1),loc_here(2),tidx));

            rfmicro(loc) = nanmean((mhw_fmicro - clim_fmicro).* 100);

        end
        % 至少1个mhw事件的值才能做平均
        if sum(~isnan(rfmicro)) >= 1
            RFMicro(loc_here(1),loc_here(2)) = nanmean(rfmicro);            
        end

        % t-test
        if sum(~isnan(rfmicro)) > 1    %至少有两个有效值来做t检验
            [h,p] = ttest(rfmicro,0,'Alpha',0.05);
            if h==1
    ...
    MHWs_FMicro_Season{seasonal} = RFMicro;  %PFTs异常的百分比
end
toc
...
m_contourf(Lon,Lat,MHWs_FMicro_Season{1}',80,'linestyle','none');
shading interp
hold on;
% ===== 显著性黑点 =====
...
caxis([-7, 7]);
% cb=colorbar;
% set(cb,'location','eastoutside','linewidth',1.5,'fontsize',16,'edgecolor','k','FontName','Times New Roman');
...
% title('Fmicro (Xi et al.2021)','FontSize',16,'FontName','Times New Roman');


%%% PFTs summer
figure('Position',[200,200,900,800],'Color','w');
m_proj('equidistant','lon',[105 123],'lat',[0 25]);
m_contourf(Lon,Lat,MHWs_FMicro_Season{2}',80,'linestyle','none');
shading interp
hold on
% ===== 显著性黑点 =====
...
figure('Position',[200,200,900,800],'Color','w');
m_proj('equidistant','lon',[105 123],'lat',[0 25]);
m_contourf(Lon,Lat,MHWs_FMicro_Season{3}',80,'linestyle','none');
shading interp
hold on
% ===== 显著性黑点 =====
...
figure('Position',[200,200,900,800],'Color','w');
m_proj('equidistant','lon',[105 123],'lat',[0 25]);
m_contourf(Lon,Lat,MHWs_FMicro_Season{4}',80,'linestyle','none');
shading interp
hold on
% ===== 显著性黑点 =====
...
%春季
B_region1 = MHWs_FMicro_Season{1}(ix2, iy2);
B_region1 = B_region1(:);
B_region1 = B_region1(~isnan(B_region1));
%夏季
B_region2 = MHWs_FMicro_Season{2}(ix2, iy2);
B_region2 = B_region2(:);
B_region2 = B_region2(~isnan(B_region2));
%秋季
B_region3 = MHWs_FMicro_Season{3}(ix2, iy2);
B_region3 = B_region3(:);
B_region3 = B_region3(~isnan(B_region3));
%冬季
B_region4 = MHWs_FMicro_Season{4}(ix2, iy2);
B_region4 = B_region4(:);
